...
SC_CITATIONS_CSV = "./sc_citations.csv"
OHCHR_CITATIONS_CSV = "./ohchr_citations.csv"
...
def trim_contents(text, tokenizer,
    ...
    # First sentence ending after target token
...
# SORT & SPLIT DATA
# ----------------------------
def sort_and_split(resolutions_dataframe: pd.DataFrame,
                   citations_dataframe: pd.DataFrame,
                   security_council_citations: pd.DataFrame,
                   human_rights_citations: pd.DataFrame):

    res_df = resolutions_dataframe.copy()
    cit_df = citations_dataframe.copy()
    sc_df = security_council_citations.copy()
    ohchr_df = human_rights_citations.copy()
    ...
    no_cite = detail[~detail["res_id2"].isin(cit_df["res_id2_doc_giving_cite"])].copy() #general assembly
    no_cite = no_cite[~no_cite["res_id2"].isin(sc_df["res_id2"])] #security council
    no_cite = no_cite[~no_cite["res_id2"].isin(ohchr_df["res_id2"])]
    no_cite["cites"] = "None"
    ...
    sc_cit = (
        sc_df.groupby(["res_id2"], as_index=True)
        .agg(sc_cit=("sc_resolution", lambda x: "; ".join(x)))
    )

    ohchr_cit = (
        ohchr_df.groupby(["res_id2"], as_index=True)
        .agg(ohchr_cit=("ohchr_resolution", lambda x: "; ".join(x)))
    )

    cit_df = pd.merge(left=cit_df, right=sc_cit, how="left", left_on="res_id2_doc_giving_cite", right_on="res_id2")
    print(f"cit_df with columns, {cit_df.columns}, as result of UNGA and SC.\n")
    cit_df = pd.merge(left=cit_df, right=ohchr_cit, how="left", left_on="res_id2_doc_giving_cite", right_on="res_id2")
    print(f"cit_df with columns, {cit_df.columns}, as result of UNGA and SC and OHCHR.\n")

    cit_df["answer"] = (
            cit_df["answer"].fillna("") + "; " +
            cit_df["sc_cit"].fillna("") + "; " +
            cit_df["ohchr_cit"].fillna("")
    )
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Extract from the text only united nations general assembly resolution codes (A/RES/), security council resolution codes (S/RES/) and human rights council resolution codes (A/HRC/RES/)."
                    "Do not explain, do not add commentary, return only results. Use ; as delimiter and follow this order: general assembly, security council, human rights."
...
def main():
    ...
    sc_df = pd.read_csv(SC_CITATIONS_CSV)
    ohchr_df = pd.read_csv(OHCHR_CITATIONS_CSV)
    print("Data loaded!")
    ...
    # Filter the res_df and the cit_df to be use exclusively with the contents shorter than max_tokens
    ...
    train_df, val_df, test_df = sort_and_split(res_df, cit_df, sc_df, ohchr_df)
